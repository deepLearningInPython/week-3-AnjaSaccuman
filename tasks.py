...
def convolve_1d(input_array, kernel_array):
    ...
    for i in range(output_length):
        output[i] = np.sum(input_array[i:i + kernel_length] * kernel_array)
    return output
...
def convolute_2d(input_matrix, kernel_matrix):
    ...
    for i in range(output_height):
        for j in range(output_width):
            element = input_matrix[i:i + kernel_height, j:j + kernel_width]
            output[i,j] = np.sum(element * kernel_matrix)
    return output
# -----------------------------------------------